...
uint8_t Serial_SendArray_DMA(uint8_t *data, uint16_t len) {
    if (tx_busy) return 1;
    if (len > sizeof(tx_dma_buf)) return 2;
...
uint8_t Serial_SendString_DMA(char *str) {
    return Serial_SendArray_DMA((uint8_t*)str, strlen(str));
}
...
void VOFA_SendData_DMA(float d0, float d1, float d2, float d3) {
    char buf[128];
    snprintf(buf, sizeof(buf), "%f,%f,%f,%f\r\n", d0, d1, d2, d3);
    while (Serial_SendString_DMA(buf) == 1) {
        // 等待前一次发送完成（通常立即成功，极少进入循环）
    }
}
