...
//	QuickCalc_Calc(e_angle, &sin_theta, &cos_theta);

//	// 反 Park
//	float valpha = Ud * cos_theta - Uq * sin_theta;
//	float vbeta  = Ud * sin_theta + Uq * cos_theta;

//	// 反 Clarke
//	float Ua = valpha;
//	float Ub = -0.5f * valpha + 0.8660254f * vbeta;   // sqrt(3)/2 ≈ 0.8660254
//	float Uc = -0.5f * valpha - 0.8660254f * vbeta;
//	Driver_SetPhaseVoltage(Ua, Ub, Uc);
}
