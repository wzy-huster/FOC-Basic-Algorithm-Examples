...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  MAF filter_speed;
  Filter_MAF_Init(&filter_speed, 20);
...
//  PID_Init(&pid_I_q, Kp, Ki, Kd, maxI, maxOut);
  ...
  while (1)
  {
	  Driver_SetVelocityOL(10.0f, 3.0f);
	  float speed_filted = Filter_MAF_Update(&filter_speed, speed);
	  VOFA_SendData(speed_filted, 0.0, 0.0, 0.0);
//	// 上位机发送dq指令
//	static float data;
//	static char type;
