...
static float bias_pos;  // 角度修正值（用于对轴）

// 对轴的初始化函数，在程序最开始执行一次
void Encoder_Init(void) {
	Beep_Init();
	bias_pos = 0.0f;  // 将偏置值归零，防止干扰末尾的读取
	Driver_SetRightangledVoltage(5.0f, 0.0f, 0.0f);  // Ud = 5V，强行吸住转子
	HAL_Delay(1000);  // 延迟1s让转子稳定
	bias_pos = Encoder_GetAngle();  // 读取当前角度并丢弃（第一次读取的值为0）
	bias_pos = Encoder_GetAngle();  // 读取当前角度作为偏置值
	Beep_Set(1);
	HAL_Delay(200);  // 蜂鸣器鸣叫200ms以示修正结束
	Beep_Set(0);
	Driver_SetRightangledVoltage(0.0f, 0.0f, 0.0f);  // Ud = 0V，释放转子
}
...
float Encoder_GetAngleNoTrack(void) {
    ...
    return -(current_rad_no_track + bias_pos);  // 根据实际情况修改符号
}
...
float Encoder_GetAngle(void) {
    ...
    return -(total_rad + bias_pos);  // 根据实际情况修改符号
}

float Last_Vel_ts = 0.0f;
float Vel_Last_Angle = 0.0f;

// 在定时中断中调用此函数，传入中断频率（例如 1000 表示 1kHz 中断，dt=1ms）
float Encoder_GetVelocity(uint32_t interrupt_freq)
{
    static float last_angle = 0.0f;
    static float velocity = 0.0f;
    float now_angle = Encoder_GetAngle();

    // 第一次调用，只记录角度，返回0
    if (last_angle == 0.0f && velocity == 0.0f) {
        last_angle = now_angle;
        return 0.0f;
    }

    float dt = 1.0f / interrupt_freq;           // 固定时间间隔（秒）
    float delta_angle = now_angle - last_angle; // 角度变化（无跳变问题）
    velocity = delta_angle / dt;                // rad/s
